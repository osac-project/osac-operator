...
// SetupWithManager registers this controller with controller-runtime. Named("externalippool-feedback") distinguishes it
// from the provisioning controller, which also watches ExternalIPPool CRs. The namespace predicate limits reconciliation
...
func (r *ExternalIPPoolFeedbackReconciler) Reconcile(ctx context.Context, request ctrl.Request) (result ctrl.Result, err error) {
	...
	if !ok {
		...
		if !object.DeletionTimestamp.IsZero() && controllerutil.ContainsFinalizer(object, osacExternalIPPoolFeedbackFinalizer) {
			log.Info("CR without external IP pool ID label is being deleted, removing feedback finalizer")
		...
		log.Info(
			"There is no label containing the external IP pool identifier, will ignore it",
			"label", osacExternalIPPoolIDLabel,
		)
		return result, err
	}
...
// saveExternalIPPool persists the proto record only if it actually changed (compared via proto.Equal), avoiding
// unnecessary gRPC calls and audit log noise when the CR is reconciled but nothing is different.
func (r *ExternalIPPoolFeedbackReconciler) saveExternalIPPool(ctx context.Context, before, after *privatev1.ExternalIPPool) error {
	log := ctrllog.FromContext(ctx)

	if !equal(after, before) {
		log.Info(
			"Updating external IP pool",
			"before", before,
			"after", after,
		)
...
func (t *externalIPPoolFeedbackReconcilerTask) handleUpdate(ctx context.Context) error {
	if controllerutil.AddFinalizer(t.object, osacExternalIPPoolFeedbackFinalizer) {
...
func (t *externalIPPoolFeedbackReconcilerTask) syncPhase(ctx context.Context) {
